...
RUN mkdir -p $HOME/.cache/chroma     && echo -n 00000000-0000-0000-0000-000000000000 > $HOME/.cache/chroma/telemetry_user_id     && chown -R $UID:$GID /app $HOME

# === Install system dependencies ===
RUN if [ "$USE_OLLAMA" = "true" ]; then       apt-get update &&       apt-get install -y --no-install-recommends         git build-essential pandoc netcat-openbsd curl jq         gcc python3-dev ffmpeg libsm6 libxext6 &&       curl -fsSL https://ollama.com/install.sh | sh &&       rm -rf /var/lib/apt/lists/*;     else       apt-get update &&       apt-get install -y --no-install-recommends         git build-essential pandoc gcc netcat-openbsd curl jq         python3-dev ffmpeg libsm6 libxext6 &&       rm -rf /var/lib/apt/lists/*;     fi
...
RUN if [ "$USE_CUDA" = "true" ]; then       pip3 install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/$USE_CUDA_DOCKER_VER --no-cache-dir;     else       pip3 install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cpu --no-cache-dir;     fi
...
RUN python -c "import os; from sentence_transformers import SentenceTransformer; SentenceTransformer('/app/backend/data/cache/embedding/models/all-MiniLM-L6-v2', device='cpu')"
